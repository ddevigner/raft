...
func sendRequest(endpoint string, id int, interval com.TPInterval, wg *sync.WaitGroup){
	...
	var reply []int
	err = client.Call("PrimesImpl.FindPrimes", interval, &reply) // Llama a FindPrimes y no avanza hasta que no termina.
	if err != nil {
		log.Fatal("primes error:", err)
	}
    if len(reply) != 0 {
        fmt.Println(id, " ", time.Since(start), "\t\tCHECK")
    } else {
        fmt.Println(id, " ", time.Since(start), "\t\tNOOOO")
    }
}
