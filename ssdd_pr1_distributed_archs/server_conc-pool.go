...
* DESCRIPCIÓN: contiene la funcionalidad esencial para realizar el servidor
		correspondiente a la arquitectura cliente-servidor concurrente
		mediante un pool fijo.
*/
package main
...
func clientHandler(tasks chan net.Conn){
...
func main() {
	...
	//Lanza <pool> goroutines
	for i := 0; i < pool; i++{
		go clientHandler(tasks)
	}
	...
	for{
		...
		//Envía las diferentes conexiones a <clientHandler>
		tasks <- conn
