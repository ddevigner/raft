 ...
 * FICHERO: escritor.go
 * DESCRIPCIÓN: Implementacion del escritor del sistema Lector-Escritor.
 * ****************************************************************************************/
 package main
 ...
 func main() {
	...
	ra := ra.New(me, 1, os.Args[3],"WRITE_RA")
	fm := fm.New(me, os.Args[2], os.Args[4],"WRITE_FM")
	tests := 1
	wait := 3
	time.Sleep(time.Duration(wait)*time.Second)
	for i := 0; i < tests; i++ {
		ra.PreProtocol()
		fm.Write(input, os.O_CREATE|os.O_RDWR, 0, os.SEEK_END) /** <-- **/ //UNEXPECTED IF USE os.O_APPEND. Read golang official documentation.
		time.Sleep(time.Duration(wait)*time.Second)
		ra.PostProtocol()
	}	 
	ra.WaitBarrier()
